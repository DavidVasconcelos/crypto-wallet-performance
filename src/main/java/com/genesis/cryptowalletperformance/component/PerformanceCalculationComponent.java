...
public class PerformanceCalculationComponent {
    ...
    public CryptoPerformance calculate(List<Asset> assets, List<CryptoAssetData> cryptoAssets) {
        var cryptoPerformance = initCryptoPerformance();
        assets.forEach(asset -> {
            ...
            var percent = cryptoAsset.priceUsd.divide(asset.getPrice(), RoundingMode.HALF_UP)
                    .setScale(2, RoundingMode.HALF_UP);
    ...
    private CryptoPerformance initCryptoPerformance() {
        return CryptoPerformance
                .builder()
